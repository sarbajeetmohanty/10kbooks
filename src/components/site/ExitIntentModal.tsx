...
export function ExitIntentModal({
  ...
  useEffect(() => {
    ...
    const handlePopState = () => {
      // Re-push history state so the user remains on page with modal open
      try {
        window.history.pushState({ modalTrap: true }, "", window.location.href);
      } catch {}
      setIsOpen(true);
  ...
  return (
    ...
    >
      <div className="relative max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-3xl border-4 border-accent bg-card text-card-foreground shadow-2xl shadow-black animate-scale-in">
        {/* Close Button */}
        <button
          type="button"
        ...
        {/* Top Header Banner */}
        <div className="bg-secondary px-5 py-3.5 text-center text-secondary-foreground sm:px-6 sm:py-4">
          <span className="inline-block rounded-full bg-accent px-3 py-0.5 text-xs font-black uppercase text-accent-foreground animate-pulse">
            Special One-Time Offer
          </span>
          <h2 className="mt-1.5 text-xl font-black uppercase tracking-tight text-white sm:text-2xl md:text-3xl">
            WAIT! DON'T LEAVE EMPTY HANDED! 🎁
          </h2>
          <p className="mt-1 text-xs font-bold text-accent sm:text-sm">
            Here is a Secret ₹50 Instant Discount for Your Child!
          </p>
        </div>
        ...
        <div className="p-5 text-center sm:p-6">
          ...
          <div className="flex flex-col items-center justify-center gap-3 sm:flex-row sm:gap-4">
            <img
              ...
              alt="14000 Kids Worksheets Bundle"
              width={100}
              height={100}
              className="h-24 w-24 rounded-2xl border-2 border-accent object-cover shadow-lg sm:h-28 sm:w-28"
            />
            <div className="text-center sm:text-left">
              <p className="text-[11px] font-bold uppercase tracking-wide text-muted-foreground sm:text-xs">
                Complete 14,000+ Worksheets Bundle
              </p>
              <div className="mt-1 flex items-baseline justify-center gap-2 sm:justify-start">
                ...
                <span className="text-2xl font-black text-accent sm:text-4xl">
                  ₹{discountPrice} ONLY
                </span>
              </div>
              <span className="mt-1 inline-block rounded-md bg-cta/20 px-2 py-0.5 text-[11px] font-extrabold text-cta sm:text-xs">
          ...
          {/* Value Bullet Points */}
          <ul className="mt-4 space-y-2 rounded-2xl bg-background/50 p-3.5 text-left text-xs font-semibold sm:mt-5 sm:p-4 sm:text-sm">
            <li className="flex items-center gap-2">
              <span className="font-bold text-accent">✔</span>
              <span>14,000+ Printable PDF Worksheets (Ages 2 to 7)</span>
            </li>
            <li className="flex items-center gap-2">
              <span className="font-bold text-accent">✔</span>
            ...
            <li className="flex items-center gap-2">
              <span className="font-bold text-accent">✔</span>
              <span>All 4 Free Bonuses (Montessori + Stories + eBooks)</span>
            </li>
          </ul>
          ...
          <div className="mt-5 sm:mt-6">
            <a
              href={checkoutUrl}
              onClick={() => setIsOpen(false)}
              className="btn-cta block w-full rounded-2xl py-3.5 text-center text-base font-black uppercase tracking-wider shadow-lg transition-transform duration-200 animate-cta-pulse hover:scale-[1.02] active:scale-[0.98] sm:text-lg cursor-pointer"
            >
